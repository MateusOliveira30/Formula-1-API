...
import {validadeDriverInfo, validadeUpdateDriverInfo, validadePosition} from '../inputValidation.js'
...
router.get( '/standings/:position', (req, res) => {
    ...
    const { error } = validadePosition(position, drivers.length);
    if(error){
...
router.post( '/', (req, res) => {
    ...
    const { error } = validadeDriverInfo(req.body);
    if(error){
        res.status(400).send(error);
        return;
    }
...
router.put( '/:id', (req, res) => {
    ...
    const { error } = validadeUpdateDriverInfo(req.body);
    if(error){
        res.status(400).send(error);
        return;
    }
